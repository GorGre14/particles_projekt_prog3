# Razred: SimulationUI (`SimulationUI.java`)

## Vloga v simulaciji
Grafični vmesnik: izris delcev in kontrole za zagon. Sekvenčni in vzporedni način tečeta **v tem procesu**; porazdeljeni potrebuje več MPI procesov, zato ga vmesnik zažene s skripto, ki odpre svoje okno.

## Kontrole

| Kontrola | Pomen |
|---|---|
| **Delci** | število delcev |
| **Cikli** | število ciklov |
| **Način** | Sekvenčna / Vzporedna (niti) / Porazdeljena (MPI) |
| **Procesi** | število MPI procesov — vidno **samo** pri porazdeljenem načinu |
| **Start** | zažene simulacijo v izbranem načinu |
| **Reset** | ustavi simulacijo in ponastavi delce na začetno stanje |

## Izris
```java
g.setColor(p.getCharge() > 0 ? Color.RED : Color.BLUE);
```
Rdeči krogi so pozitivno, modri negativno nabiti delci. Izris osvežuje `javax.swing.Timer` na EDT z nastavljeno hitrostjo (privzeto 60 FPS), neodvisno od simulacijske niti.

> [!NOTE]
> **Zakaj GUI ne zamrzne?**
> Izris poteka na Swing Event Dispatch Thread, simulacija pa v svoji niti. EDT ne opravlja nobenih fizikalnih izračunov — le periodično kliče `repaint()`.

## Zagon porazdeljenega načina
> [!IMPORTANT]
> Porazdeljene simulacije ni mogoče zagnati kot novo nit znotraj trenutne JVM, ker mora MPI okolje obstajati **že ob zagonu programa** (`MPI.Init()` je prvi klic v `DistributedRunner`).

Postopek:
1. Ob izbiri načina **Porazdeljena (MPI)** se pokaže polje **Procesi**.
2. Klik na **Start** pokaže potrditveno okno s **točnim ukazom**, ki bo izveden:
   ```
   ./run_distributed.sh [P] --particles [N] --cycles [C] --ui true
   ```
3. Po potrditvi `ProcessBuilder` zažene skripto. Ta poišče `JAVA_HOME` in `MPJ_HOME`, prevede projekt in ga požene z `mpjrun.sh` v `P` procesih.
4. Proces z rangom 0 odpre **novo okno**.

Izhod skripte je preusmerjen v konzolo (`inheritIO()`), tako da so morebitne napake takoj vidne.
