# Razred: SimulationRunner (`SimulationRunner.java`)

## Vloga v simulaciji
Vstopna točka za **sekvenčni in vzporedni** način. Vsebuje tudi **skupno simulacijsko zanko** in generator delcev, ki ju uporablja tudi `DistributedRunner` — tako vsi trije načini tečejo po istem postopku in nad istim začetnim stanjem.

## Skupna zanka `runCycles()`
Zanka je zapisana **enkrat** in jo uporabljajo vsi trije načini:
...
for (int cycle = 0; cycle < params.getNumCycles(); cycle++) {
    ...
    if (params.isEnableUI()) {
        simulation.syncToParticles(particles);   // samo za izris
        Thread.sleep(UI_FRAME_DELAY_MS);
    }
}
```

Parameter `printResults` pove, ali naj ta proces izpiše rezultat — pri MPI ga izpiše samo rang 0.

> [!NOTE]
> **Zakaj `syncToParticles()` samo pri vklopljenem vmesniku?**
> Prepis stanja v `Particle` objekte je potreben le, kadar podatke kdo gleda. V meritvah (`--ui false`) bi ta prepis meril ceno prepisovanja namesto ceno računanja.

## Deterministično generiranje delcev
```java
public static final long RANDOM_SEED = 42L;
```
Fiksen seed zagotavlja, da vsi trije načini **startajo iz enakega stanja**. Brez tega ne bi mogli potrditi, da dajejo enak rezultat, niti pošteno primerjati časov.

## Upravljanje simulacijske niti
Simulacija teče v **ločeni niti**, da grafični vmesnik ostane odziven:

- `restartSimulation()` — ustavi morebitno tekočo simulacijo, ponastavi delce na začetno stanje in zažene nov tek
- `stopSimulation()` — prekine nit, počaka nanjo in sprosti vire simulacije (npr. thread pool)

> [!NOTE]
> **Zakaj simulacija ne teče na EDT?**
> Če bi izračun tekel na Swing Event Dispatch Thread, bi bil vmesnik med računanjem zamrznjen. Izris poteka na EDT prek `javax.swing.Timer`, izračun pa v svoji niti.

## Argumenti ukazne vrstice
Bere jih `SimulationParameters.fromArgs()`, ki jo uporabljata **oba** vstopna razreda, zato so opcije v vseh načinih enake.
